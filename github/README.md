# Zenkai GitHub Action

A GitHub Action that integrates [Zenkai](#) directly into your GitHub workflow.

Mention `/opencode` in your comment, and Zenkai will execute tasks within your GitHub Actions runner.

## Features

#### Explain an issue

Leave the following comment on a GitHub issue. Zenkai will read the entire thread, including all comments, and reply with a clear explanation.
...
Leave the following comment on a GitHub issue. Zenkai will create a new branch, implement the changes, and open a PR with the changes.
...
Leave the following comment on a GitHub PR. Zenkai will implement the requested change and commit it to the same PR.
...
Leave a comment directly on code lines in the PR's "Files" tab. Zenkai will automatically detect the file, line numbers, and diff context to provide precise responses.
...
When commenting on specific lines, Zenkai receives:
...
2. Run:
   ...
   - `MODEL`: The model used by Zenkai. Same as the `MODEL` defined in the GitHub workflow.
   ...
   - `/path/to/opencode`: Path to your cloned Zenkai repo. `bun /path/to/opencode/github/index.ts` runs your local version of Zenkai.
