...
- For local scope, path is `<worktree>/.zenkai` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.zenkai`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.zenkai`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.zenkai` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@opencode-ai/plugin` into that dir and writes:
...
- Local plugins persist installed themes under the local `.zenkai/themes` area near the plugin config source.
...
- Local smoke plugin: `.zenkai/plugins/tui-smoke.tsx`
- Local vim plugin: `.zenkai/plugins/tui-vim.tsx`
- Local smoke config: `.zenkai/tui.json`
- Local smoke theme: `.zenkai/plugins/smoke-theme.json`
