...
Hay dos tipos de agentes en Zenkai; agentes primarios y subagentes.
...
Zenkai viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
Zenkai viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

Zenkai viene con dos agentes primarios integrados y tres subagentes integrados.
...
Un agente de solo lectura para investigar documentación externa y dependencias. Úsalo cuando necesites clonar el repositorio de una dependencia en la caché administrada de Zenkai, inspeccionar el código fuente de una librería o contrastar el código local con implementaciones upstream sin modificar tu espacio de trabajo.
...
Configure agentes en su archivo de configuración `zenkai.json`:
...
Si no se especifica ninguna temperatura, Zenkai utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global Zenkai como para la configuración específica del proyecto.
...
El ID del modelo en su configuración Zenkai usa el formato `provider/model-id`. Por ejemplo, si usa [Zenkai Zen](/docs/zen), usaría `zenkai/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `zenkai models` para ver una lista de los modelos disponibles.
:::
...
:::tip
¿Tiene un agente que le gustaría compartir? [Enviar un PR](#).
:::
