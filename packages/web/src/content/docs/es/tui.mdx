---
title: TUI
description: Usando la interfaz de usuario del terminal Zenkai.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Zenkai proporciona una interfaz de terminal interactiva o TUI para trabajar en sus proyectos con un LLM.

Al ejecutar Zenkai se inicia TUI para el directorio actual.
...
Cuando utilice Zenkai TUI, puede escribir `/` seguido de un nombre de comando para ejecutar acciones rápidamente. Por ejemplo:
...
Agregue un proveedor a Zenkai. Le permite seleccionar entre proveedores disponibles y agregar sus claves API.
...
Salga de Zenkai. _Alias_: `/quit`, `/q`
...
Esto es independiente de `zenkai.json`, que configura el comportamiento del servidor/tiempo de ejecución.
...
- `leader_timeout`: controla cuánto tiempo espera Zenkai después de la leader key. El valor predeterminado es `2000`.
...
- `sound_pack`: ID del sound pack que se usará. El valor predeterminado es `zenkai.default`.
