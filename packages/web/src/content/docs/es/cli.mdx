---
title: CLI
description: Opciones y comandos de la CLI de Zenkai.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de Zenkai de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
zenkai
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con Zenkai mediante programación.
...
Inicie la interfaz de usuario del terminal Zenkai.
...
La CLI de Zenkai también tiene los siguientes comandos.

---

### agent

Administrar agentes para Zenkai.
...
Conecte una terminal a un servidor backend Zenkai que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
zenkai attach [url]
```

Esto permite usar TUI con un backend remoto Zenkai. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `zenkai`) |
...
Zenkai funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `zenkai auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/zenkai/auth.json`.

```bash
zenkai auth login
```

Cuando se inicia Zenkai, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en Zenkai.
...
Ejecute zenkai en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `zenkai serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor zenkai en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `zenkai`) |
...
Inicie un servidor Zenkai sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
zenkai serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad zenkai sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `zenkai`).
...
Enumere todas las sesiones Zenkai.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones Zenkai.
...
Importe datos de sesión desde un archivo JSON o una URL compartida Zenkai.

```bash
zenkai import <file>
```

Puede importar desde un archivo local o una URL compartida Zenkai.
...
Inicie un servidor Zenkai sin cabeza con una interfaz web.

```bash
zenkai web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a Zenkai a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `zenkai`).
...
Desinstale Zenkai y elimine todos los archivos relacionados.
...
Actualiza zenkai a la última versión o a una versión específica.
...
La CLI de Zenkai toma las siguientes banderas globales.
...
Zenkai se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `zenkai`) |
