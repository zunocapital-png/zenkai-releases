...
description: Cree herramientas que LLM pueda llamar en zenkai.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de zenkai como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.zenkai/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/zenkai/tools/`.
...
```ts title=".zenkai/tools/database.ts" {1}
...
```ts title=".zenkai/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".zenkai/tools/add.py"
import sys
...
```ts title=".zenkai/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".zenkai/tools/add.py")
