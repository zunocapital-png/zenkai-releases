...
Puede agregar herramientas externas a Zenkai utilizando el _Model Context Protocol_ o MCP. Zenkai admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Zenkai Config](#) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/zenkai`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Zenkai maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Zenkai:
...
Si el servidor requiere autenticación, Zenkai le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `zenkai mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Zenkai almacenará los tokens de forma segura en `~/.local/share/zenkai/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Zenkai, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Zenkai como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Zenkai a su cuenta Sentry.
