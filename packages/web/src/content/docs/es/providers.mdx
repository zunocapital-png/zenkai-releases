---
title: Proveedores
description: Usando cualquier proveedor LLM en Zenkai.
---

import config from "../../../../config.mjs"
export const console = config.console

Zenkai usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Zenkai.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Zenkai
configuración.
...
Zenkai Zen es una lista de modelos proporcionados por el equipo Zenkai que han sido
probado y verificado para funcionar bien con Zenkai. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Zenkai Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione zenkai y diríjase a [#).
...
Funciona como cualquier otro proveedor en Zenkai y su uso es completamente opcional.

---

## Zenkai Go

Zenkai Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Zenkai que han sido
probado y verificado para funcionar bien con Zenkai.

1. Ejecute el comando `/connect` en TUI, seleccione `Zenkai Go` y diríjase a [#).
...
Para usar Amazon Bedrock con Zenkai:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta zenkai:
   ...
   Para una configuración persistente o específica del proyecto, utilice `zenkai.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Zenkai no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar zenkai para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que zenkai funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración zenkai.
...
para almacenar el token en el almacenamiento de autenticación zenkai.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Zenkai utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Zenkai
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `zenkai.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [zenkai-gitlab-auth](https://www.npmjs.com/package/zenkai-gitlab-auth).

##### Configuración

Personalizar a través de `zenkai.json`:
...
Para utilizar su suscripción GitHub Copilot con zenkai:
...
Para utilizar Google Vertex AI con Zenkai:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta zenkai.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de zenkai, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [zenkai-helicone-session](https://github.com/H2Shami/zenkai-helicone-session) para registrar automáticamente cada conversación Zenkai como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Zenkai enumerada como una sesión separada.
...
Puede configurar zenkai para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar zenkai para usar modelos locales a través de LM Studio.
...
Puedes configurar zenkai para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Zenkai. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/zenkai) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Zenkai:
...
3. Copie la clave API para usarla en Zenkai.
...
6. **Importante**: Antes de usar modelos en la nube en Zenkai, debe obtener la información del modelo localmente:
...
Zenkai Zen es una lista de modelos probados y verificados proporcionada por el equipo Zenkai. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Zenkai Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Zenkai Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Zenkai API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración zenkai.
...
5. También puedes personalizarlos a través de tu configuración zenkai. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Zenkai:
...
También puede personalizar modelos a través de su configuración zenkai. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con zenkai. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `zenkai.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Zenkai comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `zenkai auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración zenkai y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración zenkai.
