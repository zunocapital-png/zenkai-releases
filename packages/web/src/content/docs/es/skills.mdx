...
Las habilidades del agente permiten a Zenkai descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Zenkai busca estas ubicaciones:

- Configuración del proyecto: `.zenkai/skills/<name>/SKILL.md`
- Configuración global: `~/.config/zenkai/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Zenkai sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.zenkai/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/zenkai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.zenkai/skills/git-release/SKILL.md` así:
...
Zenkai enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `zenkai.json`:
...
**Para agentes integrados** (en `zenkai.json`):
