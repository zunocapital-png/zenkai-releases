---
title: Servidor
description: Interactuar con el servidor zenkai a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `zenkai serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente zenkai puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `zenkai.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `zenkai`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `zenkai serve` como a `zenkai web`.
...
Cuando ejecuta `zenkai`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor zenkai para interactuar con zenkai mediante programación.
:::

Esta arquitectura permite que zenkai admita múltiples clientes y le permite interactuar con zenkai mediante programación.

Puede ejecutar `zenkai serve` para iniciar un servidor independiente. Si tienes el
zenkai TUI ejecutándose, `zenkai serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Zenkai [IDE](/docs/ide).
...
El servidor zenkai expone las siguientes API.
