---
title: Complementos
description: Escriba sus propios complementos para extender Zenkai.
---

Los complementos le permiten extender Zenkai al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Zenkai.
...
- `.zenkai/plugins/` - Complementos a nivel de proyecto
- `~/.config/zenkai/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/zenkai/node_modules/`.
...
1. Configuración global (`~/.config/zenkai/zenkai.json`)
2. Configuración del proyecto (`zenkai.json`)
3. Directorio global de complementos (`~/.config/zenkai/plugins/`)
4. Directorio de complementos del proyecto (`.zenkai/plugins/`)
...
Zenkai ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK zenkai para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar zenkai.
...
:::note
Si está utilizando la aplicación de escritorio Zenkai, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que zenkai lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a zenkai:
...
El ayudante `tool` crea una herramienta personalizada a la que zenkai puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para zenkai junto con las herramientas integradas.
