...
Zenkai Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en Zenkai. Te suscribes a Zenkai Go y
...
Zenkai Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

Zenkai Go funciona como cualquier otro proveedor en Zenkai.

1. Inicias sesión en **<a href={console}>Zenkai Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Zenkai Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a Zenkai Go.
:::
...
Zenkai Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de Zenkai
usa el formato `zenkai-go/<model-id>`. Por ejemplo, para Kimi K3, usarías
`zenkai-go/kimi-k3` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con Zenkai.
