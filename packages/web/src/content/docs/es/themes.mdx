...
Con Zenkai puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Zenkai usa nuestro propio tema `zenkai`.
...
Zenkai viene con varios temas integrados.
...
- Quiere que Zenkai coincida con la apariencia de su terminal
...
Zenkai admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/zenkai/themes/*.json` o `$XDG_CONFIG_HOME/zenkai/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.zenkai/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.zenkai/themes/*.json`
