...
Postoje dvije vrste agenata u Zenkai: primarni agenti i podagenti.
...
Zenkai dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Zenkai dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Zenkai dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Zenkai-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `zenkai.json`:
...
- Globalno: `~/.config/zenkai/agents/`
- Po projektu: `.zenkai/agents/`
...
Ako temperatura nije navedena, Zenkai koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Zenkai konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Zenkai konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Zenkai Zen](/docs/zen), koristili biste `zenkai/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `zenkai models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](#).
:::
