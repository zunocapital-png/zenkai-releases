---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Zenkai.
---

Dodaci vam omogućavaju da proširite Zenkai spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Zenkai.
...
- `.zenkai/plugins/` - Dodaci na nivou projekta
- `~/.config/zenkai/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/zenkai/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/zenkai/zenkai.json`)
2. Konfiguracija projekta (`zenkai.json`)
3. Globalni direktorij dodataka (`~/.config/zenkai/plugins/`)
4. Direktorij dodataka projekta (`.zenkai/plugins/`)
...
Zenkai pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Zenkai.
...
Spriječite zenkai da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u zenkai:
...
Pomoćnik `tool` kreira prilagođeni alat koji zenkai može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za zenkai zajedno sa ugrađenim alatima.
