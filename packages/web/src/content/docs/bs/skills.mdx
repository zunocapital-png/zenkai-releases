...
Agent skills omogucavaju Zenkai da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Zenkai pretrazuje ove lokacije:

- Konfiguracija projekta: `.zenkai/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/zenkai/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Zenkai ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.zenkai/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/zenkai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.zenkai/skills/git-release/SKILL.md` ovako:
...
Zenkai navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `zenkai.json`:
...
**Za ugradene agente** (u `zenkai.json`):
