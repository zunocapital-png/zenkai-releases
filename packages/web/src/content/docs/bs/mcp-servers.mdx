...
Možete dodati vanjske alate u Zenkai koristeći _Model Context Protocol_, ili MCP. Zenkai podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Zenkai Config](#) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/zenkai`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Zenkai automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Zenkai će:
...
Ako server zahtijeva autentifikaciju, Zenkai će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `zenkai mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Zenkai sigurno čuva tokene u `~/.local/share/zenkai/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Zenkai, zajedno s ugrađenim alatima. Možete njima upravljati kroz Zenkai konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Zenkai sa vašim Sentry nalogom.
