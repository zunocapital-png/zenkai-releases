...
Da biste otklonili probleme s Zenkai, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\zenkai\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `zenkai --log-level DEBUG`.

---

## Pohrana

Zenkai pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/zenkai/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\zenkai`
...
Zenkai Desktop pokreće lokalni Zenkai server (`zenkai-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Zenkai` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/zenkai/zenkai.jsonc` (ili `~/.config/zenkai/zenkai.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/zenkai/zenkai.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\zenkai\zenkai.jsonc`
...
Zenkai također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/zenkai/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\zenkai\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Zenkai mogao ponovo izgraditi.

1. Potpuno zatvorite Zenkai Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/zenkai`
- **Linux**: obrišite `~/.cache/zenkai` (ili pokrenite `rm -rf ~/.cache/zenkai`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\zenkai`

3. Ponovo pokrenite Zenkai Desktop.

---

### Rješavanje problema sa vezom na serveru

Zenkai Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `zenkai.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, Zenkai Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Zenkai karakteristikama.

---

### Obavještenja se ne prikazuju

Zenkai Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Zenkai u postavkama vašeg OS-a, i
...
1. Zatvorite Zenkai Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Zenkai Desktop aplikacije):

- `zenkai.settings.dat` (URL zadanog servera za desktop)
- `zenkai.global.dat` i `zenkai.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `zenkai models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\zenkai`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Zenkai dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\zenkai`

2. Ponovo pokrenite Zenkai da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Zenkai da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Zenkai će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
