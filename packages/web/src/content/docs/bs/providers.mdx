---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Zenkai.
---

import config from "../../../../config.mjs"
export const console = config.console

Zenkai koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Zenkai konfiguraciji.
...
u `~/.local/share/zenkai/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Zenkai
config.
...
Zenkai Zen je lista modela koje je obezbedio Zenkai tim koji su bili
testirano i potvrđeno da dobro radi sa Zenkai. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Zenkai Zen.
:::
...
Radi kao i svaki drugi provajder u Zenkai i potpuno je opcionalan za korištenje.

---

## Zenkai Go

Zenkai Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Zenkai tim i koji su testirani i verificirani da dobro rade s Zenkai-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Zenkai Go` i idite na [#).
...
Da biste koristili Amazon Bedrock s Zenkai:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete zenkai:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `zenkai.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Zenkai.
:::
...
Možete konfigurirati zenkai za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi zenkai ispravno radio.
   :::
...
Također možete dodati modele kroz svoju zenkai konfiguraciju.
...
da pohrani token u zenkai auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Zenkai koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Zenkai
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`zenkai.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [zenkai-gitlab-auth](https://www.npmjs.com/package/zenkai-gitlab-auth).

##### Konfiguracija

Prilagodite putem `zenkai.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s zenkai:
...
Za korištenje Google Vertex AI s Zenkai:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće zenkai.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem zenkaia, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [zenkai-helicone-session](https://github.com/H2Shami/zenkai-helicone-session) da automatski prijavite svaki Zenkai razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Zenkai razgovor naveden kao zasebna sesija.
...
Možete konfigurirati zenkai za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati zenkai za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati zenkai za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Zenkai. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/zenkai) za detalje.
:::
...
Da biste koristili Ollama Cloud s Zenkai:
...
3. Kopirajte API ključ za korištenje u Zenkai.
...
6. **Važno**: Prije upotrebe modela oblaka u Zenkai, morate lokalno povući informacije o modelu:
...
Zenkai Zen je lista testiranih i verifikovanih modela koju je obezbedio Zenkai tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Zenkai Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Zenkai Zen**.

```txt
   /connect
```

3. Unesite svoj Zenkai API ključ.
...
Također možete dodati dodatne modele putem vaše zenkai konfiguracije.
...
5. Također ih možete prilagoditi putem vaše zenkai konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju zenkai konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s zenkai-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `zenkai.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Zenkai da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `zenkai auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Zenkai konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj zenkai konfiguraciji.
