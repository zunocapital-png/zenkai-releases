...
U Zenkai mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Zenkai koristi nasu `zenkai` temu.
...
Zenkai dolazi sa vise ugradenih tema.
...
- Zele da Zenkai odgovara izgledu njihovog terminala
...
Zenkai podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/zenkai/themes/*.json` ili `$XDG_CONFIG_HOME/zenkai/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.zenkai/themes/*.json`
4. **Trenutni radni direktorij** - `./.zenkai/themes/*.json`
