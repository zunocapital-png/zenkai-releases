---
title: Server
description: Komunicirajte s zenkai serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `zenkai serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji zenkai klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `zenkai.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `zenkai`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `zenkai serve` i za `zenkai web`.
...
Kada pokrenete `zenkai`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite zenkai server za programsku interakciju sa zenkai-om.
:::

Ova arhitektura omogucava zenkai podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `zenkai serve` da startate standalone server. Ako je zenkai TUI vec pokrenut, `zenkai serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Zenkai [IDE](/docs/ide) pluginovi.
...
zenkai server izlaže sljedece API-je.
