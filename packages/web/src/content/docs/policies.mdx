---
title: Policies
description: Control which configured resources Zenkai may use.
---

Policies control whether Zenkai may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `zenkai.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Zenkai may use a resource such as an LLM provider.
...
Zenkai currently supports one policy action:
