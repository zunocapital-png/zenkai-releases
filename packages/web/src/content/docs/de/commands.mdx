...
Erstellen Sie `.zenkai/commands/test.md`:

```md title=".zenkai/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Zenkai-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Zenkai [config](/docs/config):

```json title="zenkai.jsonc" {4-12}
{
  "$schema": "#",
  "command": {
...
- Global: `~/.config/zenkai/commands/`
- Pro Projekt: `.zenkai/commands/`

```markdown title="~/.config/zenkai/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".zenkai/commands/component.md"
---
...
```md title=".zenkai/commands/create-file.md"
---
...
```md title=".zenkai/commands/analyze-coverage.md"
---
...
```md title=".zenkai/commands/review-changes.md"
---
...
```md title=".zenkai/commands/review-component.md"
---
...
Zenkai enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
