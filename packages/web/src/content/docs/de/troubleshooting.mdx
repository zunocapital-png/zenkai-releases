...
Wenn Zenkai Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\zenkai\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `zenkai --log-level DEBUG`.

---

## Speicher

zenkai speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/zenkai/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\zenkai`
...
Zenkai Desktop startet im Hintergrund einen lokalen Zenkai-Server (`zenkai-cli`-Sidecar).
...
- Nur macOS: `Zenkai`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/zenkai/zenkai.jsonc` (oder `~/.config/zenkai/zenkai.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/zenkai/zenkai.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\zenkai\zenkai.jsonc`
...
Zenkai kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/zenkai/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\zenkai\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Zenkai ihn neu aufbauen kann.

1. Beende Zenkai Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/zenkai`
- **Linux**: loesche `~/.cache/zenkai` (oder fuehre aus: `rm -rf ~/.cache/zenkai`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\zenkai`

3. Starte Zenkai Desktop neu.

---

### Server-Verbindungsprobleme beheben

Zenkai Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `zenkai.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt Zenkai Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Zenkai-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Zenkai Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Zenkai in den OS-Einstellungen aktiviert sind, und
...
1. Beende Zenkai Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Zenkai Desktop):

- `zenkai.settings.dat` (desktop default server URL)
- `zenkai.global.dat` und `zenkai.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Zenkai hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `zenkai models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\zenkai`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. zenkai installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\zenkai`

2. Starte zenkai neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt zenkai, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
zenkai erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
