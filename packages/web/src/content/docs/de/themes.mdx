...
Mit Zenkai kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Zenkai das `zenkai`-Theme.
...
Zenkai liefert mehrere eingebaute Themes mit.
...
- Zenkai optisch ans Terminal anpassen wollen
...
Zenkai unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/zenkai/themes/*.json` oder `$XDG_CONFIG_HOME/zenkai/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.zenkai/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.zenkai/themes/*.json`
