...
Agent Skills erlauben Zenkai, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Zenkai sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Zenkai vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.zenkai/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/zenkai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.zenkai/skills/git-release/SKILL.md` zum Beispiel so:
...
Zenkai listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `zenkai.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `zenkai.json`):
