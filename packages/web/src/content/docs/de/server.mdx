---
title: Server
description: Nutze den zenkai-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `zenkai serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein zenkai-Client nutzen kann.
...
Der Benutzername ist standardmaessig `zenkai`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `zenkai serve` und `zenkai web`.
...
Wenn du `zenkai` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den zenkai-Server, um zenkai programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `zenkai serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `zenkai serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Zenkai-[IDE](/docs/ide)-Plugins.
...
Der zenkai-Server stellt folgende APIs bereit.
