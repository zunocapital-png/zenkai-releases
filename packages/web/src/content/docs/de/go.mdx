...
Zenkai Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Zenkai. Du abonnierst Zenkai Go und
...
Zenkai Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Zenkai Go funktioniert wie jeder andere Provider in Zenkai.

1. Du meldest dich bei **<a href={console}>Zenkai Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Zenkai Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Zenkai Go abonnieren.
:::
...
Zenkai Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Zenkai Config verwendet das Format `zenkai-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `zenkai-go/kimi-k3` in deiner Config verwenden.
...
Wir haben Zenkai Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Zenkai zu nutzen.
