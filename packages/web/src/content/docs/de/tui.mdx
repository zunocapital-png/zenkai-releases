---
title: TUI
description: Arbeite mit Zenkai in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Zenkai bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet Zenkai die TUI im aktuellen Verzeichnis.
...
In der Zenkai-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Fuegt einen Provider zu Zenkai hinzu. Erlaubt die Auswahl aus verfuegbaren Providern und das Hinzufuegen ihrer API-Keys.
...
Beendet Zenkai. _Aliase_: `/quit`, `/q`
...
Dies ist getrennt von `zenkai.json`, welche das Server-/Runtime-Verhalten konfiguriert.
...
- `leader_timeout` - Steuert, wie lange Zenkai nach der leader key wartet. Standard ist `2000`.
...
- `sound_pack` - Zu verwendende sound pack ID. Standard ist `zenkai.default`.
