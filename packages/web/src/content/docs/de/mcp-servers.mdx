...
Sie können externe Tools zu Zenkai hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Zenkai unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Zenkai Config](#) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/zenkai`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Zenkai übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Zenkai Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Zenkai beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `zenkai mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Zenkai die Token sicher in `~/.local/share/zenkai/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Zenkai verfügbar. Sie können sie also wie jedes andere Tool über die Zenkai-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Zenkai mit Ihrem Sentry-Konto zu verbinden.
