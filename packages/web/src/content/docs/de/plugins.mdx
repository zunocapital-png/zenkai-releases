---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Zenkai zu erweitern.
---

Mit Plugins können Sie Zenkai erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Zenkai zu ändern.
...
- `.zenkai/plugins/` – Plugins auf Projektebene
- `~/.config/zenkai/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/zenkai/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/zenkai/zenkai.json`)
2. Projektkonfiguration (`zenkai.json`)
3. Globales Plugin-Verzeichnis (`~/.config/zenkai/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.zenkai/plugins/`)
...
Zenkai führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Zenkai-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Zenkai erweitern können.
...
:::note
Wenn Sie die Zenkai-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Zenkai `.env`-Dateien liest:
...
Plugins können Zenkai auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Zenkai aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Zenkai zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](#).
