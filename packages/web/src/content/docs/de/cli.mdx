---
title: CLI
description: Zenkai CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der Zenkai CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
zenkai
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit Zenkai interagieren.
...
Starten Sie die Terminalbenutzeroberfläche Zenkai.
...
Der Zenkai CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für Zenkai verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden Zenkai-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
zenkai attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend Zenkai. Zum Beispiel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `zenkai`) |
...
Zenkai wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `zenkai auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/zenkai/auth.json` gespeichert.

```bash
zenkai auth login
```

Wenn Zenkai startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in Zenkai sehen möchten.
...
Führen Sie Zenkai im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `zenkai serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden Zenkai-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `zenkai`)                  |
...
Starten Sie einen Headless-Zenkai-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
zenkai serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf Zenkai-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `zenkai`).
...
Listen Sie alle Zenkai-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre Zenkai-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer Zenkai-Freigabe-URL.

```bash
zenkai import <file>
```

Sie können aus einer lokalen Datei oder einer Zenkai-Freigabe-URL importieren.
...
Starten Sie einen Headless-Zenkai-Server mit einer Weboberfläche.

```bash
zenkai web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf Zenkai zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `zenkai`).
...
Deinstallieren Sie Zenkai und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert Zenkai auf die neueste Version oder eine bestimmte Version.
...
Der Zenkai CLI akzeptiert die folgenden globalen Flags.
...
Zenkai kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `zenkai`) |
