...
For at fejlfinde problemer med Zenkai, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\zenkai\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `zenkai --log-level DEBUG`.

---

## Lagring

zenkai gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/zenkai/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\zenkai`
...
Zenkai Desktop kører en lokal Zenkai-server (`zenkai-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Zenkai`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/zenkai/zenkai.jsonc` (eller `~/.config/zenkai/zenkai.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/zenkai/zenkai.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\zenkai\zenkai.jsonc`
...
Zenkai kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/zenkai/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\zenkai\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Zenkai kan genopbygge den.

1. Afslut Zenkai Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/zenkai`
- **Linux**: slet `~/.cache/zenkai` (eller kør `rm -rf ~/.cache/zenkai`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\zenkai`

3. Start Zenkai Desktop på ny.

---

### Løs problemer med serverforbindelse

Zenkai Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `zenkai.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver Zenkai Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Zenkai.

---

### Meddelelser vises ikke

Zenkai Desktop viser kun systemvarsler når:

- varsler er aktiveret for Zenkai i dine OS-indstillinger, og
...
1. Afslut Zenkai Desktop.
2. Find og slet disse filer (de findes i Zenkai Desktop-appens datamappe):

- `zenkai.settings.dat` (skrivebordsstandardserver URL)
- `zenkai.global.dat` og `zenkai.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Zenkai:
...
For at finde ud af hvilke modeller du har adgang til, kør `zenkai models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\zenkai`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. zenkai installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\zenkai`

2. Start zenkai på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge zenkai til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
zenkai vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
