...
description: Brug Zenkai i enhver ACP-kompatibel editor.
---

Zenkai understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge Zenkai via ACP, konfigurer din editor til at køre kommandoen `zenkai acp`.

Kommandoen starter Zenkai som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'Zenkai'-agent i AI Chat-agentvælgeren.
...
For at bruge Zenkai som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge Zenkai som ACP-agent til chat.
...
Zenkai fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din Zenkai-konfiguration
