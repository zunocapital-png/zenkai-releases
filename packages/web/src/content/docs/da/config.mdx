---
title: Konfiguration
description: Ved at bruge Zenkai JSON konfig.
---

Du kan konfigurere Zenkai ved hjælp af en JSON-konfigurationsfil.

---

## Format

Zenkai understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/zenkai`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/zenkai/zenkai.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`zenkai.json` i projekt) - projektspecifikke indstillinger
5. **`.zenkai` mapper** - agent, kommandoer, plugins
...
:::note
`.zenkai` og `~/.config/zenkai` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/zenkai`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Zenkai-konfiguration i `~/.config/zenkai/zenkai.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `zenkai.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Zenkai starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
modes og plugins ligesom standard `.zenkai` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.zenkai` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`#).
...
Du kan konfigurere serverindstillinger for kommandoerne `zenkai serve` og `zenkai web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Zenkai-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `zenkai.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Zenkai-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Zenkai at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Zenkai-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/zenkai/agents/` eller `.zenkai/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Zenkai falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`zenkai run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/zenkai/commands/` eller `.zenkai/commands/`. [Learn more here](/docs/commands).
...
Zenkai vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard zenkai **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Zenkai med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.zenkai/plugins/` eller `~/.config/zenkai/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Zenkai til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
