...
Agentferdigheter lar Zenkai oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Zenkai søker etter disse stedene:

- Prosjektkonfigurasjon: `.zenkai/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/zenkai/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Zenkai opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.zenkai/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/zenkai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.zenkai/skills/git-release/SKILL.md` slik:
...
Zenkai viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `zenkai.json`:
...
**For innebygde agenter** (i `zenkai.json`):
