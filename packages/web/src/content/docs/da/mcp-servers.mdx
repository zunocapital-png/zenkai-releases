...
Du kan tilføje eksterne værktøjer til Zenkai ved hjælp af _Model Context Protocol_ eller MCP. Zenkai understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Zenkai Config](#) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/zenkai`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Zenkai håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Zenkai:
...
Hvis serveren kræver godkendelse, vil Zenkai bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `zenkai mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Zenkai opbevare tokens sikkert i `~/.local/share/zenkai/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Zenkai-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Zenkai til din Sentry-konto.
