---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Zenkai.
---

import config from "../../../../config.mjs"
export const console = config.console

Zenkai bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Zenkai-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/zenkai/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Zenkai-konfigurationen.
...
Zenkai Zen er en liste over modeller leveret af Zenkai-teamet, som er blevet testet og verificeret til at fungere godt med Zenkai. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Zenkai Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg zenkai og gå til [#).
...
Det fungerer som alle andre udbydere i Zenkai og er helt valgfrit at bruge.

---

## Zenkai Go

Zenkai Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Zenkai-teamet, som er testet og verificeret til at fungere godt med Zenkai.

1. Kør kommandoen `/connect` i TUI, vælg `Zenkai Go`, og gå til [#).
...
Sådan bruger du Amazon Bedrock med Zenkai:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører zenkai:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `zenkai.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Zenkai understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere zenkai til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at zenkai skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem zenkai-konfigurationen.
...
gemme tokenet i zenkai auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Zenkai bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Zenkai
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`zenkai.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [zenkai-gitlab-auth](https://www.npmjs.com/package/zenkai-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `zenkai.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Zenkai:
...
Sådan bruger du Google Vertex AI med Zenkai:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem zenkai, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [zenkai-helicone-session](https://github.com/H2Shami/zenkai-helicone-session) plugin for automatisk at logge hver Zenkai-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Zenkai-samtale opført som en separat session.
...
Du kan konfigurere zenkai til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere zenkai til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere zenkai til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Zenkai. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/zenkai) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Zenkai:
...
3. Kopier API-nøglen til brug i Zenkai.
...
6. **Vigtigt**: Før du bruger skymodeller i Zenkai, skal du hente modelinformationen lokalt:
...
Zenkai Zen er en liste over testede og verificerede modeller leveret af Zenkai-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Zenkai Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Zenkai Zen**.

   ```txt
   /connect
   ```

3. Indtast Zenkai API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem zenkai-konfigurationen.
...
5. Du kan også tilpasse dem gennem zenkai-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Zenkai:
...
Du kan også tilpasse modeller gennem zenkai-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med zenkai. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `zenkai.json`-filen i projektkataloget:
...
`limit`-felterne lader Zenkai forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `zenkai auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek zenkai-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i zenkai-konfigurationen.
