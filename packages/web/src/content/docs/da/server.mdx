---
title: Server
description: Interager med zenkai-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `zenkai serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en zenkai-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `zenkai.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `zenkai`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `zenkai serve` og `zenkai web`.
...
Når du kører `zenkai` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug zenkai-serveren til at interagere med zenkai programmatisk.
:::

Denne arkitektur lader zenkai understøtte flere klienter og lader dig interagere med zenkai programmatisk.

Du kan køre `zenkai serve` for at starte en selvstændig server. Hvis du har
zenkai TUI kørende, vil `zenkai serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Zenkai [IDE](/docs/ide) plugins.
...
Zenkai-serveren viser følgende API'er.
