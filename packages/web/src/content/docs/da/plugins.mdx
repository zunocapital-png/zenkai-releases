---
title: Plugins
description: Skriv dine egne plugins for at udvide Zenkai.
---

Plugins giver dig mulighed for at udvide Zenkai ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Zenkais standardadfærd.
...
- `.zenkai/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/zenkai/node_modules/`.
...
1. Global konfiguration (`~/.config/zenkai/zenkai.json`)
2. Projektkonfiguration (`zenkai.json`)
3. Global plugin-mappe (`~/.config/zenkai/plugins/`)
4. Projekt plugin bibliotek (`.zenkai/plugins/`)
...
Zenkai kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En zenkai SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide zenkai.
...
:::note
Hvis du bruger Zenkai desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå zenkai i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til zenkai:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som zenkai kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for zenkai sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](#) for detaljer.
