...
Med Zenkai kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Zenkai vårt eget `zenkai`-tema.
...
Zenkai støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/zenkai/themes/*.json` eller `$XDG_CONFIG_HOME/zenkai/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.zenkai/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.zenkai/themes/*.json`
