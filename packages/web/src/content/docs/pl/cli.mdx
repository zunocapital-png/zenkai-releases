---
title: CLI
description: Opcje i polecenia wiersza poleceń Zenkai.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI Zenkai domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
zenkai
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać Zenkai programowo.
...
Uruchamia interfejs terminalowy (TUI) Zenkai.
...
Interfejs CLI Zenkai zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami Zenkai.
...
Dołącz terminal do już działającego serwera backendu Zenkai uruchomionego za pomocą `serve` lub `web`.

```bash
zenkai attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem Zenkai. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `zenkai`) |
...
Zenkai integruje się z wieloma dostawcami. Możesz użyć `zenkai auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/zenkai/auth.json`.

```bash
zenkai auth login
```

Podczas uruchamiania Zenkai ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w Zenkai.
...
Uruchom Zenkai w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `zenkai serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera Zenkai (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `zenkai`) |
...
Uruchom serwer Zenkai (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
zenkai serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API Zenkai bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `zenkai`).
...
Pokaż statystyki wykorzystania tokenów i sesji Zenkai.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania Zenkai.
...
Uruchom serwer Zenkai z interfejsem internetowym.

```bash
zenkai web
```

Uruchamia to serwer HTTP i udostępnia Zenkai przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `zenkai`).
...
Odinstaluj Zenkai i usuń wszystkie powiązane pliki.
...
Aktualizuj Zenkai do najnowszej lub określonej wersji.
...
Interfejs CLI Zenkai przyjmuje następujące flagi globalne dla każdego polecenia.
...
Zenkai można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `zenkai`)    |
