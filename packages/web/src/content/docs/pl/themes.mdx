...
Dzięki zenkai możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie zenkai używa naszego własnego motywu `zenkai`.
...
zenkai ma kilka wbudowanych motywów.
...
- Chcą, aby Zenkai pasował do wyglądu ich terminala
...
zenkai obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/zenkai/themes/*.json` lub `$XDG_CONFIG_HOME/zenkai/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.zenkai/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.zenkai/themes/*.json`
