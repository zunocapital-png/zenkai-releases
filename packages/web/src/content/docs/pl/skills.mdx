...
Umiejętności agenta pozwalają zenkai odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
zenkai przeszukuje te lokalizacje:

- Project config: `.zenkai/skills/<name>/SKILL.md`
- Global config: `~/.config/zenkai/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu zenkai przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.zenkai/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/zenkai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.zenkai/skills/git-release/SKILL.md` w ten sposób:
...
zenkai wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `zenkai.json`:
...
**Dla agentów wbudowanych** (w `zenkai.json`):
