---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w zenkai.
---

import config from "../../../../config.mjs"
export const console = config.console

zenkai używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji zenkai.
...
w `~/.local/share/zenkai/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim zenkai
config.
...
Zenkai Zen to lista modeli dostarczonych przez zespół zenkai, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z zenkai. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Zenkai Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Zenkai Zen` i przejdź do [#).
...
Działa jak każdy inny dostawca w zenkai i jest całkowicie opcjonalny w użyciu.

---

## Zenkai Go

Zenkai Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół zenkai, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z zenkai.

1. Uruchom polecenie `/connect` w TUI, wybierz `Zenkai Go` i przejdź do [#).
...
Aby używać Amazon Bedrock z zenkai:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania zenkai:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `zenkai.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w zenkai nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować zenkai do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod zenkai działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji zenkai.
...
zenkai używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować zenkai
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`zenkai.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [zenkai-gitlab-auth](https://www.npmjs.com/package/zenkai-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem zenkai:
...
Aby używać Google Vertex AI z zenkai:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania zenkai.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą zenkai, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [zenkai-helicone-session](https://github.com/H2Shami/zenkai-helicone-session), aby automatycznie rejestrować każdą konwersację zenkai jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja zenkai będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować zenkai tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować zenkai tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować zenkai tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem zenkai. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/zenkai).
:::
...
Aby korzystać z Ollama Cloud z zenkai:
...
3. Skopiuj klucz API do użycia w zenkai.
...
6. **Ważne**: Przed użyciem modeli chmurowych w zenkai musisz pobrać informacje o modelu lokalnie:
...
Zenkai Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół zenkai. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Zenkai Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Zenkai Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API zenkai.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji zenkai.
...
5. Możesz je także dostosować za pomocą konfiguracji zenkai. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z zenkai:
...
Możesz także dostosować modele za pomocą konfiguracji zenkai. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z zenkai. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `zenkai.json` w katalogu projektu:
...
Pola `limit` pozwalają zenkai zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `zenkai auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację zenkai i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji zenkai.
