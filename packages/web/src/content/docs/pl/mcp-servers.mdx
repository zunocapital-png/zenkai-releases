...
Możesz dodać dodatkowe informacje do zenkai za pomocą _Model Context Protocol_, w skrócie MCP. zenkai obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [zenkai Config](#) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/zenkai`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
zenkai automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, zenkai:
...
Jeśli serwer wymaga uwierzytelnienia, zenkai poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `zenkai mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji zenkai bezpieczne przechowa tokeny w `~/.local/share/zenkai/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w zenkai, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie zenkai, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć zenkai z kontem Sentry.
