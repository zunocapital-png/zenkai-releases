...
W Zenkai obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Zenkai zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Zenkai ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

Zenkai ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej Zenkai, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `zenkai.json`:
...
- Globalnie: `~/.config/zenkai/agents/`
- W projekcie: `.zenkai/agents/`
...
Jeśli nie zostanie skonfigurowana, Zenkai używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Zenkai, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Zenkai w `provider/model-id`. Na przykład, [Zenkai Zen](/docs/zen), użyj `zenkai/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `zenkai models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](#).
:::
