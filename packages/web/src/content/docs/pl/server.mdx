---
title: Serwer
description: Interakcja z serwerem zenkai za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `zenkai serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient zenkai.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `zenkai.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `zenkai` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `zenkai serve`, jak i `zenkai web`.
...
Kiedy uruchomisz `zenkai`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera zenkai do programowej interakcji z zenkai.
:::

Ta architektura umożliwia obsługę wielu klientów przez zenkai i programową interakcję z kodem otwartym.

Możesz uruchomić `zenkai serve`, aby uruchomić samodzielny serwer. Jeśli masz
zenkai TUI działa, `zenkai serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki zenkai [IDE](/docs/ide).
...
Serwer zenkai udostępnia następujące interfejsy API.
