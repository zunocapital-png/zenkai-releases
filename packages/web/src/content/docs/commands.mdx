...
Create `.zenkai/commands/test.md`:
...
You can add custom commands through the Zenkai config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Zenkai [config](/docs/config):
...
zenkai includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
