---
title: TUI
description: Using the Zenkai terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Zenkai provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Zenkai starts the TUI for the current directory.
...
When using the Zenkai TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Zenkai. Allows you to select from available providers and add their API keys.
...
This is separate from `zenkai.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Zenkai waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `zenkai.default`.
