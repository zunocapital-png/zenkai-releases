...
There are two types of agents in Zenkai; primary agents and subagents.
...
Zenkai comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Zenkai comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

Zenkai comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into Zenkai's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `zenkai.json` config file:
...
If no temperature is specified, Zenkai uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Zenkai config and the project specific config.
...
The model ID in your Zenkai config uses the format `provider/model-id`. For example, if you're using [Zenkai Zen](/docs/zen), you would use `zenkai/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `zenkai models` to see a list of the available models.
:::
