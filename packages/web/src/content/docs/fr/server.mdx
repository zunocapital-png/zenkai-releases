---
title: Serveur
description: Interagissez avec le serveur zenkai via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `zenkai serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client zenkai peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `zenkai.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `zenkai`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `zenkai serve` et à `zenkai web`.
...
Lorsque vous exécutez `zenkai`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur zenkai pour interagir avec zenkai par programmation.
:::

Cette architecture permet à zenkai de prendre en charge plusieurs clients et vous permet d'interagir avec zenkai par programme.

Vous pouvez exécuter `zenkai serve` pour démarrer un serveur autonome. Si vous avez le
zenkai TUI en cours d'exécution, `zenkai serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Zenkai [IDE](/docs/ide).
...
Le serveur zenkai expose les API suivantes.
