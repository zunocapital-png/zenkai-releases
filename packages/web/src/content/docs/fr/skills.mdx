...
Les compétences d'agent permettent à Zenkai de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Zenkai recherche ces emplacements :

- Configuration du projet : `.zenkai/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/zenkai/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Zenkai remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.zenkai/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/zenkai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.zenkai/skills/git-release/SKILL.md` comme ceci :
...
Zenkai répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `zenkai.json` :
...
**Pour les agents intégrés** (dans `zenkai.json`) :
