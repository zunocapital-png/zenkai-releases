...
Il existe deux types d'agents dans Zenkai ; agents primaires et sous-agents.
...
Zenkai est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Zenkai est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

Zenkai est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `zenkai.json` :
...
Si aucune température n'est spécifiée, Zenkai utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Zenkai et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Zenkai utilise le format `provider/model-id`. Par exemple, si vous utilisez [Zenkai Zen](/docs/zen), vous utiliserez `zenkai/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `zenkai models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](#).
:::
