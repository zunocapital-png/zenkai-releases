...
Avec Zenkai, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Zenkai utilise notre propre thème `zenkai`.
...
Zenkai est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Zenkai corresponde à l'apparence de leur terminal
...
Zenkai prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/zenkai/themes/*.json` ou `$XDG_CONFIG_HOME/zenkai/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.zenkai/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.zenkai/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/zenkai/themes
vim ~/.config/zenkai/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .zenkai/themes
vim .zenkai/themes/my-theme.json
```
