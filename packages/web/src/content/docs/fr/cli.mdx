---
title: CLI
description: Options et commandes de la CLI Zenkai.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI Zenkai démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
zenkai
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec Zenkai par programme.
...
Démarrez l’interface utilisateur du terminal Zenkai.
...
La CLI Zenkai dispose également des commandes suivantes.

---

### agent

Gérer les agents pour Zenkai.
...
Attachez un terminal à un serveur backend Zenkai déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
zenkai attach [url]
```

Cela permet d'utiliser le TUI avec un backend Zenkai distant. Par exemple :
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `OPENCODE_SERVER_USERNAME` ou `zenkai`) |
...
Zenkai est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `zenkai auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/zenkai/auth.json`.

```bash
zenkai auth login
```

Lorsque Zenkai démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans Zenkai.
...
Exécutez zenkai en mode non interactif en transmettant directement un prompt.
...
Vous pouvez également vous connecter à une instance `zenkai serve` en cours d'exécution pour éviter les temps de démarrage à froid du serveur MCP à chaque exécution :
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Connectez-vous à un serveur zenkai en cours d'exécution (par exemple, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Mot de passe d'authentification de base (par défaut `OPENCODE_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `OPENCODE_SERVER_USERNAME` ou `zenkai`) |
...
Démarrez un serveur Zenkai sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
zenkai serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'Zenkai sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `zenkai`).
...
Liste toutes les sessions Zenkai.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions Zenkai.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage Zenkai.

```bash
zenkai import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage Zenkai.
...
Démarrez un serveur Zenkai sans interface graphique avec une interface Web.

```bash
zenkai web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à Zenkai via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `zenkai`).
...
Désinstallez Zenkai et supprimez tous les fichiers associés.
...
Met à jour zenkai vers la dernière version ou une version spécifique.
...
La CLI zenkai prend les flags globaux suivants.
...
Zenkai peut être configuré à l'aide de variables d'environnement.
...
| `OPENCODE_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `zenkai`) |
