---
title: Plugins
description: Écrivez vos propres plugins pour étendre Zenkai.
---

Les plugins vous permettent d'étendre Zenkai en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Zenkai.
...
- `.zenkai/plugins/` - Plugins au niveau du projet
- `~/.config/zenkai/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/zenkai/node_modules/`.
...
1. Configuration globale (`~/.config/zenkai/zenkai.json`)
2. Configuration du projet (`zenkai.json`)
3. Répertoire global des plugins (`~/.config/zenkai/plugins/`)
4. Répertoire des plugins du projet (`.zenkai/plugins/`)
...
Zenkai exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK zenkai pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre zenkai.
...
:::note
Si vous utilisez l'application de bureau Zenkai, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez zenkai de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à zenkai :
...
L'assistant `tool` crée un outil personnalisé que zenkai peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour zenkai aux côtés des outils intégrés.
