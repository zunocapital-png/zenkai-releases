...
With Zenkai you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Zenkai uses our own `zenkai` theme.
...
Zenkai comes with several built-in themes.
...
- Want Zenkai to match their terminal's appearance
...
Zenkai supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/zenkai/themes/*.json` or `$XDG_CONFIG_HOME/zenkai/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.zenkai/themes/*.json`
4. **Current working directory** - Defined in `./.zenkai/themes/*.json`
