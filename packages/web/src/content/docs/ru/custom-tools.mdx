...
description: Создавайте инструменты, которые LLM может вызывать в zenkai.
---

Пользовательские инструменты — это создаваемые вами функции, которые LLM может вызывать во время разговоров. Они работают вместе со [встроенными инструментами ](/docs/tools) zenkai, такими как `read`, `write` и `bash`.
...
- Локально, поместив их в каталог `.zenkai/tools/` вашего проекта.
- Или глобально, поместив их в `~/.config/zenkai/tools/`.
