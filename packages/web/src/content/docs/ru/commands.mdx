...
Создайте `.zenkai/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию zenkai или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем zenkai [config](/docs/config):
...
- Глобальный: `~/.config/zenkai/commands/`
- Для каждого проекта: `.zenkai/commands/`
...
zenkai включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
