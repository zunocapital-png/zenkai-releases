---
title: Поддержка ACP
description: Используйте zenkai в любом ACP-совместимом редакторе.
---

zenkai поддерживает [Agent Client Protocol](https://agentclientprotocol.com) (ACP), что позволяет использовать его непосредственно в совместимых редакторах и IDE.
...
Чтобы использовать zenkai через ACP, настройте свой редактор для запуска команды `zenkai acp`.

Команда запускает zenkai как ACP-совместимый подпроцесс, который взаимодействует с вашим редактором через JSON-RPC через stdio.
...
Чтобы открыть его, используйте новый агент zenkai в селекторе агентов AI Chat.
...
Чтобы использовать zenkai в качестве агента ACP в [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), добавьте в конфигурацию Neovim следующее:
...
Эта конфигурация настраивает CodeCompanion для использования zenkai в качестве агента ACP для чата.
...
zenkai через ACP работает так же, как и в терминале. Поддерживаются все функции:
...
- Серверы MCP, настроенные в вашей конфигурации zenkai
