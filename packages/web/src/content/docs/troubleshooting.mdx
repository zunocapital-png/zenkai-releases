...
To debug issues with Zenkai, start by checking the logs and local data it stores on disk.
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\zenkai\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `zenkai --log-level DEBUG`.

---

## Storage

zenkai stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/zenkai/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\zenkai`
...
To uninstall the Zenkai CLI and remove its related files, run:
...
To remove Zenkai Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

Zenkai Desktop runs a local Zenkai server (the `zenkai-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Zenkai` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/zenkai/zenkai.jsonc` (or `~/.config/zenkai/zenkai.json`)
- **macOS/Linux** (older installs): `~/.local/share/zenkai/zenkai.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\zenkai\zenkai.jsonc`
...
Zenkai can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/zenkai/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\zenkai\plugins`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Zenkai can rebuild it.

1. Quit Zenkai Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/zenkai`
- **Linux**: delete `~/.cache/zenkai` (or run `rm -rf ~/.cache/zenkai`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\zenkai`

3. Restart Zenkai Desktop.

---

### Fix server connection issues

Zenkai Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `zenkai.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, Zenkai Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with Zenkai's features.

---

### Notifications not showing

Zenkai Desktop only shows system notifications when:

- notifications are enabled for Zenkai in your OS settings, and
...
1. Quit Zenkai Desktop.
2. Find and delete these files (they live in the Zenkai Desktop app data directory):

- `zenkai.settings.dat` (desktop default server URL)
- `zenkai.global.dat` and `zenkai.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with Zenkai:
...
3. Ensure you have the latest version with `zenkai upgrade`
...
To figure out what models you have access to, run `zenkai models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\zenkai`
...
If you encounter API call errors, this may be due to outdated provider packages. zenkai dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\zenkai`

2. Restart zenkai to reinstall the latest provider packages

This will force zenkai to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
zenkai will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
