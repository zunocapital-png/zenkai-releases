...
Le skill degli agenti permettono a Zenkai di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Zenkai cerca in queste posizioni:

- Config di progetto: `.zenkai/skills/<name>/SKILL.md`
- Config globale: `~/.config/zenkai/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Zenkai risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.zenkai/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/zenkai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.zenkai/skills/git-release/SKILL.md` cosi':
...
Zenkai elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `zenkai.json`:
...
**Per agenti integrati** (in `zenkai.json`):
