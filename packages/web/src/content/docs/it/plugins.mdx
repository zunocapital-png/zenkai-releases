---
title: Plugin
description: Scrivi plugin per estendere Zenkai.
---

I plugin ti permettono di estendere Zenkai agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Zenkai.
...
- `.zenkai/plugins/` - plugin a livello progetto
- `~/.config/zenkai/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/zenkai/node_modules/`.
...
1. Config globale (`~/.config/zenkai/zenkai.json`)
2. Config di progetto (`zenkai.json`)
3. Directory plugin globale (`~/.config/zenkai/plugins/`)
4. Directory plugin di progetto (`.zenkai/plugins/`)
...
Zenkai esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di zenkai per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere zenkai.
...
:::note
Se usi l'app desktop di Zenkai, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a zenkai di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a zenkai:
...
L'helper `tool` crea uno strumento personalizzato che zenkai può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in zenkai insieme agli strumenti integrati.
