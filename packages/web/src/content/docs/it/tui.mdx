---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Zenkai.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Zenkai fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Zenkai avvia la TUI per la directory corrente.
...
Quando usi la TUI di Zenkai, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Zenkai. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `zenkai.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Zenkai attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `zenkai.default`.
