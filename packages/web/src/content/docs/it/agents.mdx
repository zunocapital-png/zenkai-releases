...
In Zenkai esistono due tipi di agenti: agenti primari e subagenti.
...
Zenkai include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Zenkai include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

Zenkai include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di Zenkai, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `zenkai.json`:
...
- Globale: `~/.config/zenkai/agents/`
- Per progetto: `.zenkai/agents/`
...
Se non specifichi una temperature, Zenkai usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Zenkai sia per la config specifica del progetto.
...
L'ID modello nella configurazione Zenkai usa il formato `provider/model-id`. Per esempio, se stai usando [Zenkai Zen](/docs/zen), useresti `zenkai/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `zenkai models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](#).
:::
