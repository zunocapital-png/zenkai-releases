...
Puoi aggiungere strumenti esterni a Zenkai usando il _Model Context Protocol_, o MCP. Zenkai supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Zenkai Config](#) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/zenkai`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Zenkai gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Zenkai:
...
Se il server richiede autenticazione, Zenkai ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `zenkai mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Zenkai memorizzera' i token in modo sicuro in `~/.local/share/zenkai/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Zenkai insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Zenkai come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Zenkai al tuo account Sentry.
