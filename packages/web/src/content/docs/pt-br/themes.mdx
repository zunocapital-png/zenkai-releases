...
Com o zenkai, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o zenkai usa nosso próprio tema `zenkai`.
...
O zenkai vem com vários temas embutidos.
...
- Querem que o zenkai corresponda à aparência do seu terminal
...
O zenkai suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/zenkai/themes/*.json` ou `$XDG_CONFIG_HOME/zenkai/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.zenkai/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.zenkai/themes/*.json`
