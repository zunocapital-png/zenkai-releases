...
Para depurar problemas com o zenkai, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\zenkai\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `zenkai --log-level DEBUG`.

---

## Armazenamento

O zenkai armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/zenkai/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\zenkai`
...
O zenkai Desktop executa um servidor zenkai local (o sidecar `zenkai-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Zenkai` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/zenkai/zenkai.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\zenkai\zenkai.jsonc`
...
O zenkai também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/zenkai/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\zenkai\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o zenkai possa reconstruí-lo.

1. Saia completamente do zenkai Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/zenkai`
- **Linux**: exclua `~/.cache/zenkai` (ou execute `rm -rf ~/.cache/zenkai`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\zenkai`

3. Reinicie o zenkai Desktop.

---

### Corrigindo problemas de conexão com o servidor

O zenkai Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `zenkai.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
...
No Windows, o zenkai Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do zenkai.

---

### Notificações não aparecendo

O zenkai Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o zenkai nas configurações do seu sistema operacional, e
...
1. Saia do zenkai Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo zenkai Desktop):

- `zenkai.settings.dat` (URL do servidor padrão do desktop)
- `zenkai.global.dat` e `zenkai.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o zenkai:
...
3. Certifique-se de que você tem a versão mais recente com `zenkai upgrade`
...
Para descobrir quais modelos você tem acesso, execute `zenkai models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\zenkai`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O zenkai instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\zenkai`

2. Reinicie o zenkai para reinstalar os pacotes de provedores mais recentes

Isso forçará o zenkai a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O zenkai detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
