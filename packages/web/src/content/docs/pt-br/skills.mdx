...
As habilidades do agente permitem que o zenkai descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O zenkai pesquisa nesses locais:

- Configuração do projeto: `.zenkai/skills/<name>/SKILL.md`
- Configuração global: `~/.config/zenkai/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o zenkai sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.zenkai/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/zenkai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.zenkai/skills/git-release/SKILL.md` assim:
...
O zenkai lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `zenkai.json`:
...
**Para agentes embutidos** (em `zenkai.json`):
