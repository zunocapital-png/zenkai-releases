...
Existem dois tipos de agentes no zenkai; agentes primários e subagentes.
...
zenkai vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Zenkai vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Zenkai vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Zenkai, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `zenkai.json`:
...
- Global: `~/.config/zenkai/agents/`
- Por projeto: `.zenkai/agents/`

```markdown title="~/.config/zenkai/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o zenkai usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do zenkai quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do zenkai usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Zenkai Zen](/docs/zen), você usaria `zenkai/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="zenkai.json" {3-6,9-12}
{
  "$schema": "#",
  "tools": {
    "write": true,
...
```json title="zenkai.json" {3-5,8-10}
{
  "$schema": "#",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `zenkai models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](#).
:::
...
```markdown title="~/.config/zenkai/agents/docs-writer.md"
---
...
```markdown title="~/.config/zenkai/agents/security-auditor.md"
---
