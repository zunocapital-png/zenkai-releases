...
أنشئ الملف `.zenkai/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Zenkai أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Zenkai:
...
- على مستوى النظام: `~/.config/zenkai/commands/`
- لكل مشروع: `.zenkai/commands/`
...
يتضمن zenkai عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
