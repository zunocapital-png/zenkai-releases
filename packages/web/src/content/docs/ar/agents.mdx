...
يوجد نوعان من الوكلاء في Zenkai: وكلاء أساسيون ووكلاء فرعيون.
...
يأتي Zenkai مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي Zenkai مع ثلاثة وكلاء فرعيين مدمجين: **General** و **Explore** و **Scout**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي Zenkai مع وكيلين أساسيين مدمجين وثلاثة وكلاء فرعيين مدمجين.
...
وكيل للقراءة فقط مخصص للوثائق الخارجية وأبحاث التبعيات. استخدمه عندما تحتاج إلى استنساخ مستودع تبعية داخل ذاكرة التخزين المؤقت المُدارة في Zenkai، أو فحص الشفرة المصدرية لمكتبة، أو إجراء مراجع متقاطعة بين الشفرة المحلية والتنفيذات upstream بدون تعديل مساحة العمل الخاصة بك.
...
هيّئ الوكلاء في ملف إعدادات `zenkai.json`:
...
- عام: `~/.config/zenkai/agents/`
- لكل مشروع: `.zenkai/agents/`
...
إذا لم يتم تحديد temperature، يستخدم Zenkai القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات Zenkai العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات Zenkai الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [Zenkai Zen](/docs/zen)، فستستخدم `zenkai/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `zenkai models` لعرض قائمة بالنماذج المتاحة.
:::
...
:::tip
هل لديك وكيل تود مشاركته؟ [قدّم PR](#).
:::
