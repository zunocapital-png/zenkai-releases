...
يمكنك إضافة أدوات خارجية إلى Zenkai باستخدام _Model Context Protocol_، أو MCP. يدعم Zenkai خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Zenkai](#) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/zenkai` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Zenkai تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Zenkai بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Zenkai المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `zenkai mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Zenkai الرموز بشكل آمن في `~/.local/share/zenkai/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Zenkai إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Zenkai مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Zenkai بحساب Sentry الخاص بك.
