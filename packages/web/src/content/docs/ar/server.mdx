---
title: الخادم
description: تفاعل مع خادم zenkai عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `zenkai serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل zenkai استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `zenkai.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `zenkai`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `zenkai serve` و `zenkai web`.
...
عند تشغيل `zenkai` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم zenkai للتفاعل مع zenkai برمجيا.
:::

تتيح هذه البنية لـ zenkai دعم عدة عملاء وتمكّنك من التفاعل مع zenkai برمجيا.

يمكنك تشغيل `zenkai serve` لبدء خادم مستقل. إذا كانت واجهة zenkai في terminal (TUI)
قيد التشغيل، فسيبدأ `zenkai serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Zenkai لـ [IDE](/docs/ide).
...
يعرض خادم zenkai واجهات API التالية.
