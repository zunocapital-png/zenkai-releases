...
مع Zenkai يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Zenkai سمتنا `zenkai`.
...
- يريدون أن يطابق Zenkai مظهر terminal لديهم
...
يدعم Zenkai نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/zenkai/themes/*.json` أو `$XDG_CONFIG_HOME/zenkai/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.zenkai/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.zenkai/themes/*.json`
