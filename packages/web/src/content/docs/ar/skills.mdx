...
تتيح مهارات الوكيل لـ Zenkai اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Zenkai في هذه المواقع:

- إعدادات المشروع: `.zenkai/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/zenkai/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Zenkai صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.zenkai/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/zenkai/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.zenkai/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Zenkai المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `zenkai.json`:
...
**للوكلاء المدمجين** (في `zenkai.json`):
