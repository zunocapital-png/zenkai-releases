---
title: المزوّدون
description: استخدام أي موفّر LLM في Zenkai.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Zenkai ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Zenkai.
...
في `~/.local/share/zenkai/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Zenkai.
...
Zenkai Zen هي قائمة نماذج يوفّرها فريق Zenkai وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Zenkai. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Zenkai Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر zenkai، ثم انتقل إلى [#).
...
يعمل مثل أي مزوّد آخر في Zenkai واستخدامه اختياري بالكامل.

---

## Zenkai Go

Zenkai Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Zenkai، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Zenkai.

1. شغّل الأمر `/connect` في TUI، واختر `Zenkai Go`، ثم انتقل إلى [#).
...
لاستخدام Amazon Bedrock مع Zenkai:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل zenkai:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `zenkai.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Zenkai غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين zenkai لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل zenkai بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات zenkai.
...
تخزين الرمز في مساحة تخزين مصادقة zenkai.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Zenkai نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Zenkai
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`zenkai.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [zenkai-gitlab-auth](https://www.npmjs.com/package/zenkai-gitlab-auth).

##### التهيئة

خصّص عبر `zenkai.json`:
...
لاستخدام اشتراك GitHub Copilot مع zenkai:
...
لاستخدام Google Vertex AI مع Zenkai:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر zenkai، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [zenkai-helicone-session](https://github.com/H2Shami/zenkai-helicone-session) لتسجيل كل محادثة Zenkai تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Zenkai مدرجة كجلسة مستقلة.
...
يمكنك ضبط zenkai لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط zenkai لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط zenkai لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Zenkai. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/zenkai) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Zenkai:
...
3. انسخ مفتاح API لاستخدامه في Zenkai.
...
6. **مهم**: قبل استخدام نماذج السحابة في Zenkai، يجب سحب معلومات النموذج محليا:
...
Zenkai Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Zenkai. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Zenkai Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Zenkai Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Zenkai.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات zenkai.
...
5. يمكنك أيضا تخصيصها عبر إعدادات zenkai. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات zenkai. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع zenkai. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `zenkai.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Zenkai بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `zenkai auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات zenkai وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات zenkai.
