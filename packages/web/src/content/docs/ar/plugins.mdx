---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Zenkai.
---

تتيح لك الإضافات توسيع Zenkai عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Zenkai.
...
- `.zenkai/plugins/` - إضافات على مستوى المشروع
- `~/.config/zenkai/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/zenkai/node_modules/`.
...
1. إعدادات عامة (`~/.config/zenkai/zenkai.json`)
2. إعدادات المشروع (`zenkai.json`)
3. دليل الإضافات العام (`~/.config/zenkai/plugins/`)
4. دليل إضافات المشروع (`.zenkai/plugins/`)
...
يشغّل Zenkai الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ zenkai للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع zenkai.
...
:::note
إذا كنت تستخدم تطبيق Zenkai لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع zenkai من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى zenkai:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ zenkai استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ zenkai إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](#) للتفاصيل.
