---
title: CLI
description: أوامر وخيارات CLI في Zenkai.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ Zenkai افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
zenkai
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع Zenkai برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ Zenkai.
...
يتضمن CLI الخاص بـ Zenkai أيضا الأوامر التالية.
...
إرفاق terminal بخادم الواجهة الخلفية لـ Zenkai قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
zenkai attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ Zenkai تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `zenkai`) |
...
يعتمد Zenkai على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `zenkai auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/zenkai/auth.json`.

```bash
zenkai auth login
```

عند تشغيل Zenkai يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في Zenkai.
...
تشغيل zenkai في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `zenkai serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم zenkai قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `zenkai`) |
...
بدء خادم Zenkai بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
zenkai serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف zenkai دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `zenkai`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات Zenkai لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة Zenkai.

```bash
zenkai import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة Zenkai.
...
بدء خادم Zenkai بدون واجهة مع واجهة ويب.

```bash
zenkai web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى Zenkai عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `zenkai`).
...
إلغاء تثبيت Zenkai وإزالة جميع الملفات ذات الصلة.
...
تحديث zenkai إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر zenkai الخيارات العامة التالية.
...
يمكن تهيئة Zenkai باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `zenkai`) |
