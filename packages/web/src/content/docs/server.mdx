---
title: Server
description: Interact with zenkai server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `zenkai serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an zenkai client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `zenkai.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `zenkai`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `zenkai serve` and `zenkai web`.
...
When you run `zenkai` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the zenkai server to interact with zenkai programmatically.
:::

This architecture lets zenkai support multiple clients and allows you to interact with zenkai programmatically.

You can run `zenkai serve` to start a standalone server. If you have the
zenkai TUI running, `zenkai serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Zenkai [IDE](/docs/ide) plugins.
...
The zenkai server exposes the following APIs.
