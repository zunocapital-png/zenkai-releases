...
You can add external tools to Zenkai using the _Model Context Protocol_, or MCP. Zenkai supports both local and remote servers.
...
You can define MCP servers in your [Zenkai Config](#) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/zenkai` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Zenkai automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Zenkai will:
...
If the server requires authentication, Zenkai will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `zenkai mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Zenkai will store the tokens securely in `~/.local/share/zenkai/mcp-auth.json`.
...
Your MCPs are available as tools in Zenkai, alongside built-in tools. So you can manage them through the Zenkai config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Zenkai to your Sentry account.
