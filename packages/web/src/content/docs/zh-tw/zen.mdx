---
title: Zen
description: 由 Zenkai 提供的精選模型列表。
---
...
Zenkai Zen 是由 Zenkai 團隊提供、經過測試與驗證的模型清單。

Zen 的運作方式和 Zenkai 中的其他供應商一樣。你登入 Zenkai Zen 並取得
...
Zenkai Zen 是一個 AI 閘道，讓你可以存取這些模型。

---

## 運作方式

Zenkai Zen 的運作方式和 Zenkai 中的其他供應商一樣。

1. 你登入 **<a href={console}>Zenkai Zen</a>**，加入帳單資訊，然後複製你的 API 金鑰。
2. 你在 TUI 中執行 `/connect` 指令，選擇 Zenkai Zen，然後貼上你的 API 金鑰。
...
Zenkai 設定中的 [模型 ID](/docs/config/#models) 會使用 `zenkai/<model-id>`
格式。例如，如果是 GPT 5.5，你會在設定中使用 `zenkai/gpt-5.5`。
...
你可能會在使用紀錄中看到 Haiku、Nano 或 Flash 等[低成本模型](/docs/config/#models)。Zenkai 使用這些模型產生工作階段標題。
...
- DeepSeek V4 Flash Free 在 Zenkai 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- MiMo-V2.5 Free 在 Zenkai 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Laguna S 2.1 Free 在 Zenkai 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Ling-3.0-flash Free 在 Zenkai 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- North Mini Code Free 在 Zenkai 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Nemotron 3 Ultra Free 在 Zenkai 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Big Pickle 是一個隱身模型，在 Zenkai 上限時免費提供。團隊正在利用這段時間收集回饋並改進模型。
...
4. 保持 **無鎖定**，讓你能把它與任何其他編碼代理一起使用，同時也始終能在 Zenkai 中使用任何其他供應商。
