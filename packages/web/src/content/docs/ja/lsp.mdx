---
title: LSPサーバー
description: Zenkai は LSP サーバーと統合します。
---

Zenkai は Language Server Protocol (LSP) サーバーと統合し、診断を agent へのフィードバックとして使用できます。

---

## 組み込み

Zenkai には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
LSP が有効で Zenkai がファイルを開くと、次のようになります。
...
Zenkai 設定の `lsp` セクションを通じて LSP サーバーを有効化し、カスタマイズできます。
