---
title: プラグイン
description: Zenkai を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Zenkai を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Zenkai のデフォルトの動作を変更したりできます。
...
- `.zenkai/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/zenkai/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/zenkai/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/zenkai/zenkai.json`)
2. プロジェクト設定 (`zenkai.json`)
3. グローバルプラグインディレクトリ (`~/.config/zenkai/plugins/`)
4. プロジェクトプラグインディレクトリ (`.zenkai/plugins/`)
...
Zenkai は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Zenkai SDK クライアント。
...
ここでは、Zenkai を拡張するために使用できるプラグインの例をいくつか示します。
...
Zenkai が `.env` ファイルを読み取らないようにします。
...
プラグインは Zenkai にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Zenkai が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Zenkai で使用できます。
