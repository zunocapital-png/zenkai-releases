---
title: CLI
description: Zenkai CLI のオプションとコマンド。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Zenkai CLI は、引数なしで実行すると、デフォルトで [TUI](/docs/tui) を開始します。

```bash
zenkai
```

ただし、このページに記載されているようにコマンドも受け入れます。これにより、Zenkai をプログラム的に操作できるようになります。
...
Zenkai ターミナルユーザーインターフェイスを開始します。
...
Zenkai CLI には次のコマンドもあります。

---

### agent

Zenkai のエージェントを管理します。
...
`serve` または `web` コマンドを使用して起動された、すでに実行中の Zenkai バックエンドサーバーにターミナルを接続します。

```bash
zenkai attach [url]
```

これにより、リモート Zenkai バックエンドで TUI を使用できるようになります。例えば：
...
| <nobr><code>{"--username"}</code></nobr> | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `zenkai`） |
...
Zenkai は [Models.dev](https://models.dev) のプロバイダーリストを利用しているため、`zenkai auth login` を使用して、使用したいプロバイダーの API キーを構成できます。これは `~/.local/share/zenkai/auth.json` に保存されます。

```bash
zenkai auth login
```

Zenkai が起動すると、認証情報ファイルからプロバイダーがロードされます。また、環境またはプロジェクト内の `.env` ファイルで定義されたキーがあるかどうかも確認します。
...
`--refresh` フラグを使用して、キャッシュされたモデルリストを更新します。これは、新しいモデルがプロバイダーに追加され、それを Zenkai で確認したい場合に便利です。
...
プロンプトを直接渡して、非対話モードで zenkai を実行します。
...
実行中の `zenkai serve` インスタンスにアタッチして、実行ごとの MCP サーバーのコールドブート時間を回避することもできます。
...
| <nobr><code>{"--attach"}</code></nobr>   |          | 実行中の zenkai サーバー (http://localhost:4096 など) に接続します。                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`     | Basic 認証パスワード（デフォルトは `OPENCODE_SERVER_PASSWORD`）                           |
| <nobr><code>{"--username"}</code></nobr> | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `zenkai`）         |
...
API アクセスのためにヘッドレス Zenkai サーバーを起動します。完全な HTTP インターフェイスについては、[サーバーのドキュメント](/docs/server) を確認してください。

```bash
zenkai serve
```

これにより、TUI インターフェイスを使用せずに zenkai 機能への API アクセスを提供する HTTP サーバーが起動します。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `zenkai`)。
...
Zenkai セッションのトークンの使用状況とコストの統計を表示します。
...
JSON ファイルまたは Zenkai 共有 URL からセッションデータをインポートします。

```bash
zenkai import <file>
```

ローカルファイルまたは Zenkai 共有 URL からインポートできます。
...
Web インターフェイスを使用してヘッドレス Zenkai サーバーを起動します。

```bash
zenkai web
```

これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて Zenkai にアクセスします。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `zenkai`)。
...
Zenkai をアンインストールし、関連ファイルをすべて削除します。
...
zenkai を最新バージョンまたは特定のバージョンに更新します。
...
| `OPENCODE_SERVER_USERNAME`            | 文字列   | 基本認証ユーザー名 (デフォルト `zenkai`) をオーバーライドします。 |
