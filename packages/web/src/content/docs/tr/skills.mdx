...
Ajan becerileri, zenkai'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
zenkai su konumlari tarar:

- Proje konfigurasyonu: `.zenkai/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/zenkai/skills/<name>/SKILL.md`
...
Proje yerel yollarinda zenkai, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.zenkai/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/zenkai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.zenkai/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
zenkai, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`zenkai.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`zenkai.json` icinde):
