...
zenkai ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak zenkai kendi `zenkai` temasını kullanır.
...
zenkai birden fazla yerleşik temayla gelir.
...
- zenkai'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
zenkai, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/zenkai/themes/*.json` veya `$XDG_CONFIG_HOME/zenkai/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.zenkai/themes/*.json`
4. **Geçerli çalışma dizini** - `./.zenkai/themes/*.json`
