...
zenkai'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
zenkai, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Zenkai, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Zenkai iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `zenkai.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse zenkai modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global zenkai yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
zenkai yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Zenkai Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `zenkai/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `zenkai models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](#).
:::
