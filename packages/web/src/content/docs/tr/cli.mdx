---
title: CLI
description: zenkai CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

zenkai CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
zenkai
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, zenkai ile programlı olarak etkileşim kurmanıza olanak tanır.
...
zenkai terminal kullanıcı arayüzünü başlatır.
...
zenkai CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

zenkai için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir zenkai arka uç sunucusuna bir terminal ekleyin.

```bash
zenkai attach [url]
```

Bu, TUI öğesinin uzak bir zenkai arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `zenkai`) |
...
zenkai, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `zenkai auth login` kullanabilirsiniz. Bu `~/.local/share/zenkai/auth.json`'de saklanıyor.

```bash
zenkai auth login
```

zenkai başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları zenkai'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek zenkai'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `zenkai serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir zenkai sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `zenkai`) |
...
API erişimi için headless bir zenkai sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
zenkai serve
```

Bu, TUI arayüzü olmadan zenkai işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `zenkai` şeklindedir).
...
Tüm zenkai oturumlarını listeleyin.
...
zenkai oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya zenkai paylaşımından URL oturum verilerini içe aktarın.

```bash
zenkai import <file>
```

Yerel bir dosyadan veya zenkai paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir zenkai sunucusu başlatın.

```bash
zenkai web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla zenkai'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `zenkai` şeklindedir).
...
zenkai'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
zenkai'u en son sürüme veya belirli bir sürüme günceller.
...
zenkai CLI aşağıdaki global bayrakları destekler.
...
zenkai ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `zenkai`) |
