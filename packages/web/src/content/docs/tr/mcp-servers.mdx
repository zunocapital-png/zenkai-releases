...
_Model Bağlam Protokolü_ veya MCP kullanarak zenkai'a harici araçlar ekleyebilirsiniz. zenkai hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [zenkai Config](#)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/zenkai` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
zenkai, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde zenkai şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa zenkai, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `zenkai mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından zenkai, belirteçleri `~/.local/share/zenkai/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra zenkai'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi zenkai yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve zenkai'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
