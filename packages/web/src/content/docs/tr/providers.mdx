---
title: Sağlayıcılar
description: zenkai'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

zenkai, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı zenkai yapılandırmanızda yapılandırın.
...
`~/.local/share/zenkai/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları zenkai'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Zenkai Zen, zenkai ekibi tarafından sağlanan modellerin bir listesidir.
zenkai ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Zenkai Zen ile başlamanızı öneririz.
:::
...
zenkai'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Zenkai Go

Zenkai Go, zenkai ile iyi çalıştığı test edilmiş ve doğrulanmış, zenkai ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Zenkai Go`'yu seçin ve [#) adresine gidin.
...
Amazon Bedrock'u zenkai ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   zenkai'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `zenkai.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi zenkai'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
zenkai'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   zenkai'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
zenkai, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. zenkai'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`zenkai.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [zenkai-gitlab-auth](https://www.npmjs.com/package/zenkai-gitlab-auth) ana sayfasında.

##### Yapılandırma

`zenkai.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi zenkai'la kullanmak için:
...
Google Vertex AI'yi zenkai ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları zenkai çalıştırılırken ayarlayın.
...
Helicone'un zenkai aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her zenkai görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [zenkai-helicone-session](https://github.com/H2Shami/zenkai-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her zenkai görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
zenkai'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla zenkai'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla zenkai'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini zenkai için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/zenkai)'a bakın.
:::
...
Ollama Cloud'u zenkai ile kullanmak için:
...
3. zenkai'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: zenkai'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Zenkai Zen, zenkai ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Zenkai Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Zenkai Zen**'i arayın.

   ```txt
   /connect
   ```

3. zenkai API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   zenkai yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca zenkai yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri zenkai yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı zenkai ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `zenkai.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları zenkai'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `zenkai auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için zenkai yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, zenkai yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
