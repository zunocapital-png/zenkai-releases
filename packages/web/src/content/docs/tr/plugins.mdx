---
title: Eklentiler
description: Zenkai'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Zenkai'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Zenkai'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.zenkai/plugins/` - Proje düzeyinde eklentiler
- `~/.config/zenkai/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/zenkai/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/zenkai/zenkai.json`)
2. Project config (`zenkai.json`)
3. Global eklenti dizini (`~/.config/zenkai/plugins/`)
4. Proje eklenti dizini (`.zenkai/plugins/`)
...
Zenkai bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Zenkai SDK istemcisi.
...
Zenkai'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Zenkai masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Zenkai'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Zenkai'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Zenkai'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Zenkai'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](#) bakın.
