...
Zenkai Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Zenkai'daki diğer sağlayıcılar gibi çalışır. Zenkai Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Zenkai'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Zenkai Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Zenkai Go, Zenkai'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Zenkai Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Zenkai Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Zenkai Go'ya abone olabilir.
:::
...
Zenkai Go aşağıdaki limitleri içerir:
...
Zenkai yapılandırmanızdaki [model id](/docs/config/#models) formatı `zenkai-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `zenkai-go/kimi-k3` kullanmalısınız.
...
Zenkai Go'yu şu amaçlarla oluşturduk:
...
4. Zenkai ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
