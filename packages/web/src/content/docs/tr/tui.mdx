---
title: TUI
description: Zenkai terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Zenkai, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Zenkai'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Zenkai TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Zenkai'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Zenkai'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `zenkai.json` dosyasından ayrıdır.
...
- `leader_timeout` - Zenkai'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `zenkai.default`.
