...
Det er to typer agenter i Zenkai; primære agenter og underagenter.
...
Zenkai kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Zenkai kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Zenkai kommer med to innebygde primære agenter og tre innebygde underagenter.
...
En skrivebeskyttet agent for ekstern dokumentasjon og forskning på avhengigheter. Bruk denne når du trenger å klone et avhengighetsrepo inn i Zenkai sin administrerte cache, inspisere kildekoden til et bibliotek eller kryssjekke lokal kode mot upstream-implementasjoner uten å endre arbeidsområdet ditt.
...
Konfigurer agenter i konfigurasjonsfilen `zenkai.json`:
...
- Globalt: `~/.config/zenkai/agents/`
- Per prosjekt: `.zenkai/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Zenkai modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Zenkai-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Zenkai-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Zenkai Zen](/docs/zen), vil du bruke `zenkai/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `zenkai models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](#).
:::
