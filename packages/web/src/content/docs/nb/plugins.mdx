...
description: Skriv dine egne programtillegg for å utvide Zenkai.
---

Plugins lar deg utvide Zenkai ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Zenkai.
...
- `.zenkai/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/zenkai/node_modules/`.
...
1. Global konfigurasjon (`~/.config/zenkai/zenkai.json`)
2. Prosjektkonfigurasjon (`zenkai.json`)
3. Global plugin-katalog (`~/.config/zenkai/plugins/`)
4. Prosjektpluginkatalog (`.zenkai/plugins/`)
...
Zenkai kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Zenkai SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Zenkai.
...
:::note
Hvis du bruker Zenkai-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Zenkai fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Zenkai:
...
`tool`-hjelperen lager et tilpasset verktøy som Zenkai kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Zenkai sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](#) for detaljer.
