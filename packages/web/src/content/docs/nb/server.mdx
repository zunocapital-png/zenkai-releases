---
title: Tjener
description: Samhandle med Zenkai-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `zenkai serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Zenkai-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `zenkai.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `zenkai` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `zenkai serve` og `zenkai web`.
...
Når du kjører `zenkai` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Zenkai-serveren til å samhandle med Zenkai programmatisk.
:::

Denne arkitekturen lar Zenkai støtte flere klienter og lar deg samhandle med Zenkai programmatisk.

Du kan kjøre `zenkai serve` for å starte en frittstående server. Hvis du har
Zenkai TUI kjører, vil `zenkai serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Zenkai [IDE](/docs/ide) plugins.
...
zenkai-serveren viser følgende APIer.
