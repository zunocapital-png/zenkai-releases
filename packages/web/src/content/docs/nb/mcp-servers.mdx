...
Du kan legge til eksterne verktøy i Zenkai ved å bruke _Model Context Protocol_ eller MCP. Zenkai støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Zenkai Config](#) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="zenkai.jsonc" {6}
{
  "$schema": "#",
  "mcp": {
    "name-of-mcp-server": {
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/zenkai`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Zenkai håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Zenkai:
...
Hvis serveren krever autentisering, vil Zenkai be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `zenkai mcp auth <server-name>`.
...
```bash
zenkai mcp auth my-oauth-server
```
...
```bash
zenkai mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Zenkai lagre tokens sikkert i `~/.local/share/zenkai/mcp-auth.json`.
...
zenkai mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Zenkai, sammen med innebygde verktøy. Så du kan administrere dem gjennom Zenkai-konfigurasjonen som alle andre verktøy.
...
```json title="zenkai.json" {11, 14-18}
{
  "$schema": "#",
  "mcp": {
    "my-mcp": {
...
```bash
zenkai mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Zenkai til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Zenkai-samtale oppført som en separat økt.
