---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Zenkai.
---

import config from "../../../../config.mjs"
export const console = config.console

Zenkai bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Zenkai-konfigurasjonen.
...
i `~/.local/share/zenkai/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Zenkai
konfig.
...
Zenkai Zen er en liste over modeller levert av Zenkai-teamet som har vært
testet og verifisert for å fungere godt med Zenkai. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Zenkai Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Zenkai Zen` og gå til [#).
...
Det fungerer som alle andre leverandører i Zenkai og er helt valgfritt å bruke.

---

## Zenkai Go

Zenkai Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Zenkai-teamet som har vært
testet og verifisert for å fungere godt med Zenkai.

1. Kjør kommandoen `/connect` i TUI, velg `Zenkai Go`, og gå til [#).
...
Slik bruker du Amazon Bedrock med Zenkai:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Zenkai:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `zenkai.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i zenkai støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere zenkai til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Zenkai skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Zenkai-konfigurasjonen.
...
for å lagre token i Zenkai auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Zenkai bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Zenkai
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`zenkai.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [zenkai-gitlab-auth](https://www.npmjs.com/package/zenkai-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `zenkai.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Zenkai:
...
Slik bruker du Google Vertex AI med Zenkai:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Zenkai.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Zenkai, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [zenkai-helicone-session](https://github.com/H2Shami/zenkai-helicone-session) plugin for automatisk å logge hver Zenkai-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Zenkai-samtale oppført som en separat økt.
...
Du kan konfigurere Zenkai for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Zenkai for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Zenkai for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Zenkai. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/zenkai) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Zenkai:
...
3. Kopier API-nøkkelen for bruk i Zenkai.
...
6. **Viktig**: Før du bruker skymodeller i Zenkai, må du hente modellinformasjonen lokalt:
...
Zenkai Zen er en liste over testede og verifiserte modeller levert av Zenkai-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Zenkai Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Zenkai Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Zenkai API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Zenkai-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Zenkai-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Zenkai:
...
Du kan også tilpasse modeller gjennom Zenkai-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Zenkai. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `zenkai.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Zenkai forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `zenkai auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Zenkai-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Zenkai-konfigurasjonen.
