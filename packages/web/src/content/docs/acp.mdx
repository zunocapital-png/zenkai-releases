---
title: ACP Support
description: Use Zenkai in any ACP-compatible editor.
---

Zenkai supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use Zenkai via ACP, configure your editor to run the `zenkai acp` command.

The command starts Zenkai as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'Zenkai' agent in the AI Chat agent selector.
...
To use Zenkai as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use Zenkai as the ACP agent for chat.
...
Zenkai works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your Zenkai config
