...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".zenkai", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".zenkai")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".zenkai").toReversed()
    ...
    // global config, project files, then `.zenkai` files.
