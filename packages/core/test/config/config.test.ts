...
describe("Config", () => {
  ...
  it.live("loads global, ancestor, and .zenkai configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".zenkai"), { recursive: true })
            await fs.mkdir(path.join(directory, ".zenkai"), { recursive: true })
            await Promise.all([
              ...
              fs.writeFile(path.join(root, ".zenkai", "opencode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".zenkai", "opencode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".zenkai")),
              AbsolutePath.make(path.join(directory, ".zenkai")),
            ])
