...
export function SessionComposerRegion(props: {
  ...
  // Modo "sin permisos" (autoApprove): si el usuario lo activa, cada solicitud
  // de permiso se aprueba automaticamente. Aditivo y defensivo — si esta
  // apagado (default), el flujo normal muestra el dock de permisos.
  createEffect(() => {
    try {
      if (!settings.permissions.autoApprove()) return
      if (!controller.state.permissionRequest()) return
      if (controller.state.permissionResponding()) return
      controller.onResponseSubmit()
      controller.state.decide("once")
    } catch {
      /* nunca romper el render por el auto-approve */
    }
  })
